...
2. **TVA (décision d'expert)** : conversion du réel en HT par **taux de catégorie** (table du classeur, colonne « récupérable oui/non »). Le champ `vat_amount` de Qonto ne sert que de contrôle (47 % de couverture, erreurs dans les deux sens). La TVA **non récupérable** reste en charge (coût définitif). Point de contrôle avant publication : nature des honoraires holdings (14 208 € de TVA) à confirmer avec le comptable.
