...
- **I1** : les exclusions PCG (TVA reversée, IS — cf section 2.1) ne s'appliquent qu'au réel Qonto (`computeChargesHybride`, boucle N/N-1), jamais au budget CR_Prev (`fetchAndParseCRPrev`). Aujourd'hui sans effet : la ligne budget « IS » de −15 852 € est en janvier-juillet, hors fenêtre prévisionnelle (mois courant et suivants). Mais si une ligne IS ou TVA venait à être budgétée sur un mois ≥ mois courant, elle entrerait dans `totalCharges` ET double-compterait avec `computeIS` (qui recalcule l'IS séparément). Piège à traiter si le budget CR_Prev évolue en ce sens.
