...
**Précision d'implémentation (2026-08-05, révisée sans changer le comportement livré)** : l'énumération multi-exercice de `computePrimesChargeSchedule` (server.js, R8) rejoue `computePrimePayments` pour chaque année de SIGNATURE `y` dans `[N-2, N-1, N]`. Sans garde-fou, la même provision glissante (datée au trimestre courant réel, indépendamment de `y`) serait retenue à CHAQUE rejeu, donc comptée plusieurs fois. La règle réellement implémentée (`selectChargeEntriesForExercice`, testée dans `utils/kpiCompute.test.js`) n'accepte donc une entrée de statut provisoire que si son année de signature `y` est **égale** à l'exercice courant `N` ; les millésimes `N-2`/`N-1` sont écartés pour ce statut (les entrées non provisoires, elles, ne sont filtrées que sur leur `dateCharge`). Deux conséquences non triviales, documentées en détail en §10 points 7 et 8 :
- un deal signé en `N-1`, toujours pas facturé quand `N-1` se clôture, ne porte plus AUCUNE charge en `N` (ni provision, ni charge réelle) tant qu'il n'est pas facturé ;
- une provision posée en `N` n'est jamais reprise si la charge réelle correspondante tombe en `N+1` (mois de `N` déjà figés côté write-back), donc la même prime peut peser sur deux exercices consécutifs.
...
7. **Deal signé en N-1, toujours pas facturé en N : aucune charge en N.** Conséquence directe du filtre de provision de R3 (une provision n'est retenue que pour le millésime de signature égal à l'exercice courant) : dès que l'exercice de signature d'un deal non facturé se clôture, ce deal sort du calcul de charge de l'exercice suivant. Il ne porte donc aucune charge en N : ni provision (son exercice de signature N-1 est clos), ni charge réelle (l'acompte n'est toujours pas facturé). Il réapparaîtra automatiquement, en charge « due »/« à venir » (R2), le jour de la facturation de l'acompte, dans l'exercice où cette facturation tombe (potentiellement bien après N-1).
8. **Provision posée en N reprise en charge réelle en N+1 : la même prime pèse sur deux exercices.** Si une provision est posée au T4 de l'exercice N (mois de charge décembre N, déjà écrit dans le Sheet et clos au moment où l'acompte est enfin facturé), puis l'acompte est facturé début N+1 (charge réelle théorique en N+1 selon R2), rien n'annule la provision de N : le figement du write-back interdit d'écrire dans un exercice déjà clos (cf. point 3), et R3 ne relit jamais les exercices antérieurs pour reprendre une provision devenue obsolète. Le résultat de N reste donc chargé d'une provision jamais reprise, et celui de N+1 porte en plus la charge réelle : la même prime pèse, en cumulé, sur deux exercices consécutifs. Aucun correctif automatique prévu dans ce lot ; à traiter manuellement si constaté en recette.
