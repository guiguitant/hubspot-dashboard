...
- `verifierLiasse(liasse)` : gardes d'intégrité de saisie, retourne un tableau d'anomalies (vide attendu) : (G1) totalProduitsExploitation − chargesExploitation = resultatExploitation à ±2 € (arrondis de la liasse elle-même) ; (G2) resultatExploitation − impotSurBenefices = resultatNet à ±2 € ; (G3, ajoutée en revue : ferme la seule faute de saisie silencieuse possible sur productionVendue) productionVendue + subventionsExploitation + (autresProduits || 0) + productionImmobilisee = totalProduitsExploitation à ±2 €, avec un message qui N'ACCUSE PAS à tort (« vérifier la saisie, ou une rubrique de produits non modélisée » : le total 2033-B peut contenir des ventes de marchandises, de la production stockée ou des reprises non modélisées) ; champs numériques requis présents. Une liasse en anomalie est SERVIE quand même, avec ses anomalies exposées (jamais de silence). PROCÉDURE DE SAISIE ANNUELLE : le schéma ne porte que `cir` ; un exercice comportant un CII nécessite d'étendre le schéma (sinon il disparaîtrait de la zone ancrée).
