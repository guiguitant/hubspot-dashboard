...
**Limite connue n° 2 · configuration des primes non millésimée** (relevée en revue finale, hors lot) : `kpi_prime_config` ne stocke qu'une seule ligne (`id = 'default'`) portant `tiers`, `resultatAnnuel`, `gateTrimestriel` et `rates`. Ces paramètres sont donc GLOBAUX, pas rattachés à un exercice : pendant la période de grâce, l'exercice N-1 est rejoué avec la config telle qu'elle est AU MOMENT DU CALCUL. Concrètement, si l'utilisateur met à jour ses paliers ou ses taux début janvier (cas normal : nouvelle année, nouveaux objectifs), le recalcul de décembre N-1 pendant la grâce réécrit rétroactivement l'exercice clos avec les paramètres de N. Le correctif propre serait une config millésimée (une ligne par exercice, lecture par année cible), avec migration des valeurs actuelles vers l'exercice courant. Décision : HORS LOT (aucun code écrit pour ce point dans le chantier produits-et-suivis). Contournement d'ici là : ne modifier `kpi_prime_config` qu'APRÈS le 20 janvier, une fois la grâce terminée et l'exercice N-1 définitivement figé.
