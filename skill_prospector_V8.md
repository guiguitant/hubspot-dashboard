...
# Releaf Prospector — Instructions opérationnelles v8
...
const existingLinkedinUrls = new Set(
  existing.map(p => (p.linkedin_url || '').toLowerCase().replace(/\/$/, '').split('?')[0])
);
...
⚠️ Normalisation : `.toLowerCase().replace(/\/$/, '').split('?')[0]` — identique au serveur (`normalizeLinkedinUrl`).
...
Le prospect sera créé en statut `'Profil à valider'` et les données manquantes pourront être complétées manuellement ou lors d'une session ultérieure.
...
try {
  ...
  const normalizedUrl = (profile.linkedin_url || '').toLowerCase().replace(/\/$/, '').split('?')[0];
...
for (let i = 0; i < toSync.length; i += 25) {
  ...
  _summary.profiles_submitted += batch.length;           // envoyés au sync
  _summary.profiles_created += result.created || 0;      // réellement créés
  _summary.profiles_rejected_duplicates += result.skipped || 0; // doublons serveur
  _totalSubmitted += result.created || 0;
  console.log(`Batch sync: ${result.created} créés, ${result.skipped} skippés (doublons serveur)`);
}
...
Créés en "Profil à valider" : ${_summary.profiles_created}
